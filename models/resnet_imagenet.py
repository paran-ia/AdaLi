...
from spikingjelly.activation_based.layer import Conv2d, AvgPool2d, Linear, AdaptiveAvgPool2d,Flatten
...
class ScaledWSConv2d(Conv2d):
    ...
    def forward(self, x):
        out = []
        for input in x:
            out.append(F.conv2d(input, self.get_weight(), self.bias, self.stride, self.padding, self.dilation, self.groups))
...
def conv1x1(in_planes, out_planes, stride=1):
    """1x1 convolution"""
    return Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=True)
...
class SpikingNFResNet(nn.Module):
    def __init__(self, block, layers, num_classes=200,
        ...
        self.maxpool = AvgPool2d(kernel_size=3, stride=2, padding=1)
        expected_var = 1.0
...
def spiking_nfresnet34(neuron: callable=None, **kwargs):
    model = _spiking_resnet('resnet34', BasicBlock, [3, 4, 6, 3], neuron, **kwargs)
    functional.set_step_mode(model, 'm')
    return model


def spiking_nfresnet50(neuron: callable=None, **kwargs):
    model = _spiking_resnet('resnet50', Bottleneck, [3, 4, 6, 3], neuron, **kwargs)
    functional.set_step_mode(model, 'm')
    return model

if __name__ == '__main__':
    data = torch.randn(3,2,3,224,224)
    model  = spiking_nfresnet34(IF,surrogate_function=AdaLi)
